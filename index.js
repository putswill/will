...
const vcard = 'BEGIN:VCARD\n' // OM JANGAN DI UBAH OM
            ...
            + 'FN:RamlanGans\n' // PUTSWILL MODS
            + 'ORG:Owner BOT;\n' // WILIAN GUALBINO @PUTSWILL
            + 'TEL;type=CELL;type=VOICE;waid=16991541171:+55 16 99154 1171\n' // UBAH INI
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks =  `[ BEM VINDO AO GRUPO GOSTOSO(A) ${mdata.subject}* ] \n___________________________\n@${num.split('@')[0]} INTRO!! \nNOME : \nFOTO : \nIDADE : \nLEU A DESCRIÇÃO? : \nSABE CHAMAR O BOT SADSTATION?: \nVAI MAMAR O TIO WILL PARA GANHAR ADM? \nQUALQUER DÚVIDA CHAME O TIO WILL! \n *___________________________*\nFÉ NO PAI QUE O INIMIGO CAI!!`
				...
				teks = `XAU RASCUNHO DO CAPETA, NÃO ERA NEM BIM VINDO(A) NO GRUPO MESMO... @${num.split('@')[0]}😔✌* \n_PARA VOLTAR VAI TER QUE MAMAR O TIO WILL ͡° ͜ʖ ͡°_`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				wait3: '[❗] CALMA CARALHO SOU UM BOT NÃO SEU ESCRAVO',
			...
			const ownerNumber = ["5516991541171@s.whatsapp.net"] // Ganti pake nomor lu
			const ramlanNumber = ["5516991541171@s.whatsapp.net"] // JANGAN DI UBAH NGENTOD
			...
			switch(command) {
				...
				case 'info':
					...
					break
				case 'blocklist':
					teks = 'CONTATOS BLOQUEADOS PELO TIO WILL KSKS :\n'
               ...
               client.sendMessage(from, '𝗕𝗥𝗨𝗫𝗜𝗡𝗛𝗢 𝗟𝗜𝗡𝗗𝗢. <- 𝗠𝗘𝗨 𝗖𝗥𝗜𝗔𝗗𝗢 𝗣𝗗𝗣!!:*',MessageType.text, { quoted: mek} )
				...
				case 'bc':
					...
					if (mentioned.length > 1) {
						...
						for (let _ of mentioned) {
							teks += `Pessoal esse membro comum acaba de virar um escravo do tio will, respeitem o(a) novo(A) adm ok! :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(from, mentioned, true)
						client.groupRemove(from, mentioned)
					} else {
						mentions(`Pessoal esse membro comum @${mentioned[0].split('@')[0]} acaba de virar um escravo do tio will, respeitem o novo adm ok!`, mentioned, true)
				...
				case 'demote':
					...
					if (mentioned.length > 1) {
						...
						for (let _ of mentioned) {
							teks += `Pessoal esse inutil acaba de perder seu cargo admin, digitem (F) no chat ksks :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupRemove(from, mentioned)
					} else {
						mentions(`Pessoal esse inutil𝗠𝗢𝗩𝗜 𝗢 @${mentioned[0].split('@')[0]} acaba de perder seu cargo de admin, digitem  (F) no chat ksks𝗟𝗘 𝗙𝗔𝗟𝗛𝗢𝗨 𝗡𝗔 𝗠𝗜𝗦𝗦𝗔𝗢 𝗗𝗘 𝗥𝗘𝗦𝗣𝗘𝗜𝗧𝗔𝗥 𝗔𝗦 𝗥𝗘𝗚𝗥𝗔𝗦!`, mentioned, true)
				...
				case 'kick':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'REMOVI MESMO, GHOTS AQUI NAO TEM VEZ, E AINDA GOSTA DE TIKTOK KSKS :\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
						...
						mentions(`REMOVI MESMO, GHOTS AQUI NAO TEM VEZ, E AINDA GOSTA DE TIKTOK KSKS  : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'admin':
